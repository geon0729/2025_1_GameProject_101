...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Input.GetKey(KeyCode.G) && !isGrounded && gliderTimeLeft > 0)         //G키를
        {
        ...
        //점프 입력
        if (Input.GetButtonDown("Jump") && isGrounded)         //&& 두 값을 만족할때 -> (스페이스 버튼을 눌렀을때 와 isGrounded가 True 일때)
        {
    ...
    void ApplyGliderMovement(float horizontal, float vertical)
    {
        ...
        Vector3 gliderVelodcity = new Vector3(
            horizontal * gliderMoveSpeed,
            -gliderFallSpeed,
            vertical * gliderMoveSpeed
        );

        rb.velocity = gliderVelodcity;
    }
    ...
    private void OnTriggerEnter(Collider other)          //트리거 영역 안에 들어왔나를 검사하는 함수
    {
        ...
        //목적지 도착 시 종료 로그 출력
        if(other.gameObject.tag == "Door" && coinCount == totalCoins)      //모든 코인 획득후에 문으로 가면 게임 종료
        {
            Debug.Log("게임 클리어");
            //게임 완료 로직 추가 가능
        }
    ...
    void UpdateGroundedState()
    {
        ...
        else
        {
            ...
            {
                coyoteTimeCounter -= Time.deltaTime;        //시간을 지속적으로
                isGrounded = true;
            }
            else
            {
